...
\begin{soln}
	...
	\]
	By SVD, $X = U \Sigma V^T$ so $X^T X = (U \Sigma V^T)^T(U \Sigma V^T)$
	\[
	...
	\]
	\[
	\Rightarrow a_1 = \arg \max_{a_1} a_1^T V \Sigma^T \Sigma V^T a_1
	\]
	\[
	= \arg \max_{a_1} a_1^T \Sigma^T \Sigma a_1 \text{ since } V \text{ is an orthogonal matrix } V^T V = I \text{ and } \text{change our optimization constraint}
	\]
	\(\Sigma^T \Sigma\) is the diagonal matrix of singular values: \(\text{diag}(\sigma_1^2, \sigma_2^2, \ldots)\) 
	such that \(\sigma_1^2 \geq \sigma_2^2 \geq \ldots \geq \sigma_r^2\).
	
	Maximizing \(a_1^T \Sigma^T \Sigma a_1\) is therefore the same as maximizing \(\frac{a_1^T \Sigma^T \Sigma a_1}{a_1^T a_1}\),
	
	Since \(a_1^T a_1 = 1\), the optimal \(a_1\) is the one in which \(a_1\) is the unit vector that maximizes the term of \(\sigma_1^2\), the first squared singular value. This \(a_1\) must be the first right singular vector.
\end{soln}
